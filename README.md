...
- More than one product type uses the same attributes, and they have to stay consistent.
...
**Variant Manager -> Variants** lists every variant in the store on its own, rather than nested under its product, so a filter applies across the whole catalog. Filters cover stock, whether inventory is tracked, and each registered attribute.

Select variants and the actions menu offers **Bulk edit field**, which writes one value to all of them. The action stays hidden until `bulkEditableVariantFields` in `config/variant-manager.php` names at least one field.
