...
class BaseFormat {
	...
	public function export($product, $variants = null, $download = false) {
		...
		if ($variants === null) $variants = $product->variants;

		// In future, this should return an error message.

		if (!$product) return null;

		if ($download) $this->controller->setDownloadableAs($product->title . '.' . $this->ext, $this->mimetype);

		if (!$download) $this->controller->returnType = $this->returnType;
	...
	public function findSKUs($items) {
		...
		foreach ($found as $variant) {
			...
			if (!array_key_exists($variant->product->id, $mapped)) $mapped[$variant->product->id] = [];
