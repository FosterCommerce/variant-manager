...
return [
	...
	'variantMaker.modeInstructions' => 'Add only creates the combinations that are missing, and leaves every existing variant unchanged. Update also rewrites existing variants with every included property. Replace does that and deletes any variant not in the generated set.',
	...
	'variantMaker.propertiesInstructions' => 'Include a property to have the Variant Maker set it. SKU and price are always set, since Commerce requires both, and a new variant is always titled. An excluded property keeps Commerce\'s default on a new variant and its current value on an existing one.',
	'variantMaker.formatTokens' => 'In a title or SKU format, {Attribute Name} becomes the option chosen under that attribute; any other text is used as typed. Left blank, a title is the options joined by a slash and a SKU is the default variant SKU followed by each option, with spaces turned into dashes.',
