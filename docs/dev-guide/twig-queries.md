...
Substitute the handle you gave your Variant Attributes field (`variantAttributes`, `myVariantAttributes`, or the handle you chose) for `variantAttributes` in the examples below.
...
- `filter values must be strings`: an associative filter had a non-string value, as in `{ 'Size': 3 }`. Quote it.
