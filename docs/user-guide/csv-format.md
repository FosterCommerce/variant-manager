...
| Column | Required | Format | Notes |
|--------|----------|--------|-------|
| `title` | No | Text | Omit the column to leave variant titles alone. On a product type with no variant title field, a blank cell means Commerce generates the title from the product type's variant title format. On a product type that has one, a blank cell means Craft assigns `Product Variant {id}`. |
...
| `availableForPurchase` | `availableForPurchase[default]` | `1` or `0`. Defaults to `1` if the column is missing and the file has another per-site column. |
| `freeShipping` | `freeShipping[default]` | `1` or `0` |
| `promotable` | `promotable[default]` | `1` or `0`. Defaults to `1` if the column is missing and the file has another per-site column. |
...
Empty cells become `emptyAttributeValue` (default: empty string).
