...
The preview and **Generate variants** both require Commerce's `commerce-editProductType` permission for the product's type, on top of plugin access. Without it, neither the preview nor the generate runs.
...
Each option can have a **Price Modifier**, set in the same place as its SKU partial. A combination's price is the base price plus the modifier of every option in it. Both use the store currency's subunits, so two decimal places for most currencies.
