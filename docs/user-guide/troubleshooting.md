...
This usually means an export was moved between environments, where the same product has a different ID, or an ID was typed by hand.
...
If you wanted to delete every existing variant first, see "Replace all variants" in [importing](./importing.md#existing-product-update-options).
...
An import usually fails because the CSV was wrong (bad filename, duplicate SKU, smart quotes, wrong attribute prefix). Retrying the job fails again with the same error.
