...
- Saving with a BOM (some apps add one to CSV UTF-8; if your app lists both, pick plain "CSV (comma-delimited)").
...
![The Variant Manager dashboard with the Upload Product button](../../resources/img/dashboard.png)
...
![The upload modal showing Create Product with a product type dropdown](../../resources/img/product-import-new.png)
...
![The upload modal showing Edit Product with the two variant update options](../../resources/img/product-import-edit.png)
...
![The upload modal confirming the import was queued](../../resources/img/product-import-success.png)
...
Failed imports show their error in the activity log. The two most common reasons are a missing `{id}__` prefix on a file meant to update, and a SKU that already exists on a different product. See [troubleshooting](./troubleshooting.md).

**Failed queue jobs**: delete them. Retrying a failed import job fails with the same error because the data in the job is the data that failed. Fix the CSV and re-upload it instead. See [cleaning up failed import jobs](./troubleshooting.md#cleaning-up-failed-import-jobs).
