...
**Delete failed jobs rather than retrying them.** A failed import almost always failed because of bad CSV data, and retrying with the same data fails the same way. Fix the source CSV and upload it again. See [cleaning up failed import jobs](./troubleshooting.md#cleaning-up-failed-import-jobs).
